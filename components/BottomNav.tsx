...
export default function BottomNav({ tabs, activeTab, onTabChange }: BottomNavProps) {
    return (
        <div className="fixed bottom-0 left-0 right-0 bg-surface border-t border-borderLight flex justify-center py-1.5 px-2 shadow-[0_-2px_12px_rgba(0,0,0,0.05)] z-[100]">
            <div className="flex gap-0.5 max-w-[600px] w-full">
                {tabs.map(t => (
                    <button key={t.key}
                        onClick={() => onTabChange(t.key)}
                        className={`flex-1 flex flex-col items-center gap-1 p-2.5 rounded-xl border-none bg-none cursor-pointer text-[11px] font-semibold transition-all relative min-w-[68px]
